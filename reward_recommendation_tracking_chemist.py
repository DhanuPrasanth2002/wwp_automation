...
config.read('reward_recommendation_tracking_chemist.ini')

class reward_recommendation_tracking_chemist(unittest.TestCase):
    ...
    def generate_random_cheque_date(self):
    ...
    def test_export_buttons(self):
        try:
            ...
            self.driver.get(config.get('DEFAULT', 'reward_recommendation_tracking_chemist_url'))
    ...
    def test_add_customer(self):
        try:
            ...
            add_customer_button = WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, config['DEFAULT']['add_button_xpath']))
            )
            ...
            # Click the dropdown box for customer
            self.wait_and_click(config.get('DEFAULT', 'customer_dropdown_xpath'))
            # Select the option for customer
            customer_option_xpath = config.get('DEFAULT', 'customer_option_xpath')
            self.wait_and_click(customer_option_xpath)
            time.sleep(1)

            # Set random cheque date
            cheque_date_input = self.wait_and_find(config.get('DEFAULT', 'cheque_date_xpath'))
            random_cheque_date = self.generate_random_cheque_date()
            cheque_date_input.send_keys(random_cheque_date)
            time.sleep(1)

            # Click the dropdown box for issue to
            self.wait_and_click(config.get('DEFAULT', 'issuedto_dropdown_xpath'))
            # Select the option for issued to
            issuedto_option_xpath = config.get('DEFAULT', 'issuedto_option_xpath')
            self.wait_and_click(issuedto_option_xpath)
            time.sleep(1)

            # Click outside the body
    ...
    def test_view_customer(self):
        try:
            ...
            mode_of_payment_field = self.wait_and_find(config.get('DEFAULT', 'mode_of_payment_xpath'))
            amount_field = self.wait_and_find(config.get('DEFAULT', 'view_amount_xpath'))
            payment_credited_date_field = self.wait_and_find(config.get('DEFAULT', 'payment_credited_date_xpath'))
            cheque_date_field = self.wait_and_find(config.get('DEFAULT', 'view_cheque_date_xpath'))
            issued_to_field = self.wait_and_find(config.get('DEFAULT', 'issuedto_xpath'))

            assert name_as_per_records_field.get_attribute("readonly") == "true", "Name As Per Records field is editable"
            assert mode_of_payment_field.get_attribute("readonly") == "true", "Mode of Payment field is editable"
            assert amount_field.get_attribute("readonly") == "true", "Amount field is editable"
            assert payment_credited_date_field.get_attribute("readonly") == "true", "Payment Credited Date field is editable"
            assert cheque_date_field.get_attribute("readonly") == "true", "Cheque Date field is editable"
            assert issued_to_field.get_attribute("readonly") == "true", "Issued To field is editable"
    ...
    def test_edit_customer(self):
        try:
            ...
            name_as_per_records_field = self.wait_and_find(config.get('DEFAULT', 'name_as_per_records_xpaths'))
            mode_of_payment_field = self.wait_and_find(config.get('DEFAULT', 'edit_mode_of_payment_xpath'))
            amount_field = self.wait_and_find(config.get('DEFAULT', 'amount_xpath'))
            payment_credited_date_field = self.wait_and_find(config.get('DEFAULT', 'edit_payment_credited_date_xpath'))
            cheque_date_field = self.wait_and_find(config.get('DEFAULT', 'cheque_date_xpath'))
            issued_to_field = self.wait_and_find(config.get('DEFAULT', 'issuedto_dropdown_xpath'))

            assert name_as_per_records_field.is_enabled(), "Name As Per Records field is not editable"
            assert mode_of_payment_field.is_enabled(), "Mode of Payment field is not editable"
            assert amount_field.is_enabled(), "Amount field is not editable"
            assert payment_credited_date_field.is_enabled(), "Payment Credited Date field is not editable"
            assert cheque_date_field.is_enabled(), "Cheque Date field is not editable"
            assert issued_to_field.is_enabled(), "Issued To field is not editable"


            cheque_date_field.clear()
            cheque_date_field.send_keys(config.get('DEFAULT', 'edit_cheque_date_field'))
            ...
            # Verify the updated values in the customer list
            self.driver.get(config.get('DEFAULT', 'reward_recommendation_tracking_chemist_url'))
            time.sleep(1)

            updated_customer = self.wait_and_find(config.get('DEFAULT', 'updated_customer_xpath'))
            assert updated_customer is not None, "Updated customer details not found in the table"
    ...
    def test_delete_customer(self):
        try:
            ...
            self.fail(f"Error during delete customer test: {e}")
...
def run_all_tests():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(reward_recommendation_tracking_chemist)
...
if __name__ == "__main__":
    ...
    test_instance = reward_recommendation_tracking_chemist()
    ...
    try:
        ...
        try:
            ...
            failed_tests += 1
        # try:
        #     test_instance.test_delete_customer()
        #     passed_tests += 1
        # except Exception as e:
        #     print(f"Test failed: test_delete_customer - {e}")
        #     failed_tests += 1

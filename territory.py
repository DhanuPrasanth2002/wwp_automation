...
config.read('territory_config.ini')
...
class territoryTests(unittest.TestCase):
    ...
    def test_export_buttons(self):
        try:
            self.driver.get(config.get('DEFAULT', 'territory_masterlist_url'))
    ...
    def test_add_territory(self):
        try:
            ...
            print("Navigated to territory Master List Page")
            ...
            self.wait_and_click(config.get('DEFAULT', 'headquarters_dropdown_xpath'))
            # Select the option for speciality
            speciality_option_xpath = config.get('DEFAULT', 'headquarters_option_value')
            self.wait_and_click(speciality_option_xpath)
            time.sleep(1)

            territory_name_input = self.wait_and_find(config.get('DEFAULT', 'territory_name_xpath'))
            self.validate_input(territory_name_input, config.get('DEFAULT', 'territory_name'), "text")

            # Click the dropdown box for territory
            self.wait_and_click(config.get('DEFAULT', 'region_dropdown_xpath'))
            # Select the option for territory
            territory_option_xpath = config.get('DEFAULT', 'region_option_value')
            self.wait_and_click(territory_option_xpath)
            time.sleep(1)
            ...
            # assert headquarters_name_input.get_attribute("value") == "", "Clear button did not reset the rep name input"
            ...
            expected_values = [
                config.get('DEFAULT', 'territory_name')
            ]
    ...
    def test_view_territory(self):
        try:
            ...
            region_field = self.wait_and_find(config.get('DEFAULT', 'region_xpath'))
            country_field = self.wait_and_find(config.get('DEFAULT', 'country_xpath'))


            assert headquarters_name_field.get_attribute("readonly") == "true", "headquarter name field is editable"
            assert territory_name_field.get_attribute("readonly") == "true", "territory name field is editable"
            assert region_field.get_attribute("readonly") == "true", "region field is editable"
            assert country_field.get_attribute("readonly") == "true", "country field is editable"
    ...
    def test_delete_territory(self):
        try:
            # Navigate to territory Master List Page
            self.driver.get(config.get('DEFAULT', 'territory_masterlist_url'))
            
            # Retrieve necessary XPaths from configuration for product entry and delete button
            territory_entry_xpath = config.get('DEFAULT', 'territory_entry_xpath')
            ...
            territory_entry = self.wait_and_find(territory_entry_xpath)
            territory_id = territory_entry.get_attribute("data-id")
            
            # Click the delete button for the product
            self.wait_and_click(delete_button_xpath)
            print("Clicked delete button for territory")
            ...
            with self.assertRaises(Exception):
                self.driver.find_element(By.XPATH, f"//table[@id='specialityTable']//tr[@data-id='{territory_id}']")
            print("speciality deleted successfully, not found in the list")
            
        except Exception as e:
            self.fail(f"Error during delete speciality test: {e}")
    
    @order(13)
    def test_rerun_add_territory(self):
        try:
            time.sleep(1)
            self.driver.get(config.get('DEFAULT', 'territory_masterlist_url'))
            self.wait_and_click(config.get('DEFAULT', 'add_button_xpath'))
    ...
    def test_edit_territory(self):
        try:
            ...
            headquarters_name_field = self.wait_and_find(config.get('DEFAULT', 'headquarters_dropdown_xpath'))
            territory_name_field = self.wait_and_find(config.get('DEFAULT', 'territory_name_xpath'))
            region_field = self.wait_and_find(config.get('DEFAULT', 'region_dropdown_xpath'))
            country_field = self.wait_and_find(config.get('DEFAULT', 'country_xpath'))


            assert headquarters_name_field.is_enabled(), "headquarters name field is not editable"
            assert territory_name_field.is_enabled(), "territory name is not editable"
            assert region_field.is_enabled(), "region field is not editable"
            assert country_field.is_enabled(), "country field is not editable"
  



            Select(headquarters_name_field).select_by_visible_text(config.get('DEFAULT', 'edit_headquarters_name'))

            territory_name_field.clear()
            territory_name_field.send_keys(config.get('DEFAULT', 'edit_territory_name'))

            Select(region_field).select_by_visible_text(config.get('DEFAULT', 'edit_region'))
    ...
    def test_dropdown_headquarters(self):
        try:
            # Step 1: Navigate to the territory list page
            self.driver.get(config.get('DEFAULT', 'territory_masterlist_url'))
            print("Navigated to Territory Master List Page")
            time.sleep(1)

            # Step 2: Navigate to the headquarters list page and click the add button
            self.driver.get(config.get('DEFAULT', 'headquarters_masterlist_url'))
            self.wait_and_click(config.get('DEFAULT', 'add_button_xpath'))
            ...
            headquarters_name = config.get('DEFAULT', 'drop_check_headquarters_name')
            print(f"Entering headquarters name: {headquarters_name}")
            headquarters_name_input.clear()
            headquarters_name_input.send_keys(headquarters_name)
            self.wait_and_click(config.get('DEFAULT', 'submit_button_xpath'))
            ...
            time.sleep(1)
            headquarters_name_option_value = config.get('DEFAULT', 'check_dropdown_headquarters_option_value')
            ...
            assert headquarters_option.text == headquarters_name, f"Expected headquarters name '{headquarters_name}' not found in the dropdown"
...
def run_all_tests():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(territoryTests)
...
if __name__ == "__main__":
    ...
    try:
        try:
            test_instance.test_add_territory()
            passed_tests += 1
        except Exception as e:
            print(f"Test failed: test_add_territory - {e}")
            failed_tests += 1
        ...
        try:
            test_instance.test_view_territory()
            passed_tests += 1
        except Exception as e:
            print(f"Test failed: test_view_territory - {e}")
            failed_tests += 1

        try:
            test_instance.test_edit_territory()
            passed_tests += 1
        except Exception as e:
        ...
        try:
            test_instance.test_delete_territory()
            passed_tests += 1
        except Exception as e:
            print(f"Test failed: test_delete_territory - {e}")
            failed_tests += 1


        try:
            test_instance.test_rerun_add_territory()
            passed_tests += 1
        except Exception as e:
            print(f"Test failed: test_dropdown_headquarters - {e}")
            failed_tests += 1
